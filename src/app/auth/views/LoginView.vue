...
import type { AuthLoginRequest, AuthLoginResponse } from '@buf/ahmeddarwish_devkit-api.bufbuild_es/devkit/v1/accounts_auth_pb';
...
const loginFormProps: AppFormProps<'login', AuthLoginRequest, AuthLoginRequest, AuthLoginResponse> = {
	context: {
		title: 'login',
		formKey: 'login',
		resetOnSuccess: true,
		...
		submitHandler: {
			...
			callback: (response) => {
				if (response.navigationBar) localStorage.setItem('sidebar', JSON.stringify(response.navigationBar));
